...
extern SplitStateView120* gSplit_123457C; extern int PanelY_76C2A0(void*,int); extern bool EffectiveFull_763170(void*);
struct XYScale120{float x,y,s;};
XYScale120 SplitDrawTransform_9E1C08(float x,float y,float s,bool status1){if(status1){s*=gSplit_123457C->scale;x=float(FullCoordTransform_76A460(gSplit_123457C,int(x)));y*=0.5f;}return{x,y,s};}
XYScale120 SplitCenteredTransform_9E2A21(float x,float s,bool status1){float y=360.0f;if(status1){s*=gSplit_123457C->scale;x=float(FullCoordTransform_76A460(gSplit_123457C,int(x)));y=float(PanelY_76C2A0(gSplit_123457C,180));}return{x,y,s};}
...
XYScale120 SplitHudAnchor_9EAE23(int x,int y,float baseScale,bool status1){if(!status1)return{float(x),float(y),baseScale};float s=gSplit_123457C->scale;return{float(FullCoordTransform_76A460(gSplit_123457C,x)),float(SplitVerticalTransform_76A4A0(gSplit_123457C,int(float(y)*s))),s};}
void ApplySplitNodeXYZScale_9EC52B(void*nodeRaw,bool status1){auto*n=static_cast<std::uint8_t*>(nodeRaw);float s=1.0f;if(status1){float x=*reinterpret_cast<float*>(n+0x50);x=float(FullCoordTransform_76A460(gSplit_123457C,int(x)));*reinterpret_cast<float*>(n+0x50)=x;s=gSplit_123457C->scale;}*reinterpret_cast<float*>(n+0x70)=s;*reinterpret_cast<float*>(n+0x74)=s;*reinterpret_cast<float*>(n+0x78)=s;}
...
void ApplySplitVerticalScale_A1E384(void*nodeRaw){auto*n=static_cast<std::uint8_t*>(nodeRaw);float&y=*reinterpret_cast<float*>(n+0x54);y*=((gSplit_123457C->scale+1.0f)*0.5f);y+=float(SplitVerticalTransform_76A4A0(gSplit_123457C,0));}
