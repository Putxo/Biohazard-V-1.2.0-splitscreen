...
extern Split* gSplit_123457C; extern int PanelY_76C2A0(Split*,int); extern bool IsFullSplitEffective(Split*);
struct Node{unsigned char p[0x50];float x,y,z;unsigned char q[0x14];float sx,sy,sz;};
void PromptGeometry_A0FD72(int baseX,int& x,int& halfGap,float& scale){if(Status_C42D90()!=1){x=baseX;return;} scale=gSplit_123457C->scale; float h=scale*360.f; x=FullCoordTransform_76A460(gSplit_123457C,baseX); halfGap=int((360.f-h)*0.5f);}
float PromptScale_A0FE14(int mode){float s=gSplit_123457C->scale*40.f; if(mode==9||mode==10)s*=0.699999988079071f; return s;}
void ScaleHudPoint_A33778(int& x,int& y,float& s){if(Status_C42D90()!=1)return; s=gSplit_123457C->scale; x=FullCoordTransform_76A460(gSplit_123457C,int(s*40.f)); y=SplitVerticalTransform_76A4A0(gSplit_123457C,int(float(y)*s));}
void TransformHudPoint_A3383F(int& x,int& y){if(Status_C42D90()!=1)return; x=FullCoordTransform_76A460(gSplit_123457C,x); y=SplitVerticalTransform_76A4A0(gSplit_123457C,int(float(y)*gSplit_123457C->scale));}
// Same native +3074 transform occurs at A3427F, A34337 and A343D2.
void ScaleHudNodeParam_A34260(Node* n,int x){if(!n)return; n->x=float(SplitCoordTransform_76A420(gSplit_123457C,x)); n->sx*=gSplit_123457C->param; n->sy*=gSplit_123457C->param;}
...
void CenteredHudNode_A5C501(Node* n){if(!n)return; n->x=float(SplitCoordTransform_76A420(gSplit_123457C,0x190)); n->y=float(PanelY_76C2A0(gSplit_123457C,180)); n->sx*=gSplit_123457C->scale; n->sy*=gSplit_123457C->scale; n->sz*=gSplit_123457C->scale;}
...
void TransformTwoPlayerHudNode_A7352D(Node* n){if(!n||Status_C42D90()!=1)return;n->x*=gSplit_123457C->scale;n->x+=gSplit_123457C->offset;n->y*=gSplit_123457C->scale;n->y=float(SplitVerticalTransform_76A4A0(gSplit_123457C,int(n->y)));n->sx=n->sy=n->sz=gSplit_123457C->scale;}
