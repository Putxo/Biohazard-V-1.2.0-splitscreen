...
// 0x00A02F20..0x00A02F3C and 0x00A02F40..0x00A02F5C.
...
// 0x00A01ABF..0x00A01AEC -- exact owner-specific arbitration condition isolated
// from its generic parent. false is the one native case in which the candidate
// is deliberately NOT copied into the parent's local selection slot.
...
static_assert(offsetof(OwnerRecord120,selection54)==0x54,"owner record selection");
static_assert(offsetof(OwnerManager120,pending)==0x5B80,"owner manager pending");
static_assert(offsetof(OwnerManager120,changed)==0x5B84,"owner manager changed");

// 0x00A037C0..0x00A038EC -- exact local-owner selector update.
// Important native edge case: when the UI cursor already equals pending,
// 0xA038B4 jumps directly to the return and intentionally leaves +5B80/+5B84
// untouched. Earlier reconstruction incorrectly cleared both fields there.
bool UpdateOwnedSelector_A037C0(OwnerManager120* self,OwnerRecord120* rec){
    ...
    if(code!=0x1B&&code!=0xDB&&code!=0xDD&&code!=0xB6){
        self->pending=-1;
        self->changed=0;
        return false;
    }
    ...
    bool routed;
    if(!sessionState)
        routed=UiRoute5750_7B5750(gUiInput_11B20C4,0,0,0x11,0);
    else if(rec->player==0)
        routed=UiRoute5750_7B5750(gUiInput_11B20C4,0,0,8,0);
    else
        routed=UiRoute5750_7B5750(gUiInput_11B20C4,9,0x11,0,0);

    if(routed)
        self->pending=UiGetSelection_7B5360(gUiInput_11B20C4,0);

    if(self->pending==-1 || !UiSelectionChanged_7B4660(gUiInput_11B20C4,0)){
        self->changed=0;
        self->pending=-1;
        return false;
    }
    ...
    if(current==pending)
        return false; // native A038B4 -> A038E7: preserve +5B80/+5B84
...
// 0x00A06A5A..0x00A06A9C -- exact owner-dependent prompt resource selection.
...
// 0x00A23090..0x00A23119 -- exact event predicate, including +0x614 owner test.
bool MenuOwnerEventPredicate_A23090(UiOwnerBase120*self,int eventId){
    if(eventId==1){
        if(!SessionHasLocalCoop_C43BB0(Session()))return false;
        return KeyboardPlayer()!=self->owner34;
    }
    if(eventId==5){
        if(SessionDisplayStatus_C42D90(Session())!=1)return false;
        auto* obj=*reinterpret_cast<void**>(gUiManager_1234578+0x3830);
        return !UiEventBlocked_728C90(obj);
    }
