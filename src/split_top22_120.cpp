...
extern void DrawNamed_9E5330(void* self,const char* key,int x,int y,
                             const void* descriptor,float scaleX,float scaleY);
...
namespace {
template <class T> static inline T& f(void* p, std::size_t off) {return *reinterpret_cast<T*>(reinterpret_cast<std::uint8_t*>(p) + off);}
...
{
    const int baseIndex = UiIndex_A17800(self);

    std::uint16_t topWord;
    if (QueryGameStatus_C42D90() == 1) {
        const int scaled = truncf(GetSplitState_123457C()->splitScale * 144.0f);
        topWord = static_cast<std::uint16_t>(SplitVerticalTransform_76A4A0(GetSplitState_123457C(),scaled));
    } else {
    ...
    int yCursor = static_cast<std::int16_t>(topWord);
    if (QueryGameStatus_C42D90() == 1) {
    ...
    const void* descriptor = UiTableEntry_112C080(i == f<std::int32_t>(self, 0x48) ? 0 : 1);

    if (i == 0) {
        const float scale = QueryGameStatus_C42D90() == 1 ? GetSplitState_123457C()->splitScale : 1.0f;
        const std::uint16_t id = ReadWord_112BE50();

        int x = QueryGameStatus_C42D90() != 1 ? 0x17C : 0x280 - truncf(GetSplitState_123457C()->splitScale2 * 260.0f);
        DrawMain_9E5E90(self, drawIndex, x, yCursor, &descriptor, id, 0x60);

        x = QueryGameStatus_C42D90() != 1 ? 0x33E : 0x280 - truncf(GetSplitState_123457C()->splitScale2 * -190.0f);
        DrawNamed_9E5330(self, Str_FA80D4(), x, yCursor, &descriptor, scale, 1.0f);

        const int ratio = (f<std::int32_t>(self, 0x190) * 0x137) / f<std::int32_t>(self, 0x194);
        int pointX = ratio + 0x2A1;
        if (QueryGameStatus_C42D90() == 1)
            pointX = truncf(static_cast<float>(pointX - 0x280) * GetSplitState_123457C()->splitScale2) + 0x280;
        DrawNamed_9E5330(self, Str_FA80CC(), pointX, yCursor + 0x11, &descriptor, scale, 1.0f);

        x = QueryGameStatus_C42D90() != 1 ? 0x33C : 0x280 - truncf(GetSplitState_123457C()->splitScale2 * -188.0f);
        DrawNamed_9E5330(self, Str_10BAC14(), x, yCursor + 0x4E, &descriptor, scale, 1.0f);
    }
    ...
    if (i == f<std::int32_t>(self, 0x48)) {
        ...
        if (current > 0) {
            ...
            int x = 0x280;
            if (QueryGameStatus_C42D90() == 1) x -= truncf(GetSplitState_123457C()->splitScale2 * -0.0f);
            List_9E5020(self, text, x, yCursor);
        }

        if (current < limit) {
            Format_E63B3E(text, Fmt_FA6C14(), UiArray_112C144(i));
            const int x = QueryGameStatus_C42D90() != 1 ? 0x3FC : 0x280 - truncf(GetSplitState_123457C()->splitScale2 * -380.0f);
            List_9E5020(self, text, x, yCursor);
        }
    }
    ...
    yCursor += baseIndex;
    yCursor += 0x82;
    if (f<std::int32_t>(self, 0x20) == 0) yCursor += baseIndex;
