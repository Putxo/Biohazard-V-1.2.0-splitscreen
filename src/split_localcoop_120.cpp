...
void PollLocalJoin_723460(AGame120* self){
 ...
 if(*reinterpret_cast<std::int32_t*>(session+0x58)!=0)return;
 auto* g=*reinterpret_cast<std::uint8_t**>(0x01249C1C);
 if(*reinterpret_cast<std::int32_t*>(g+0x48)>1)return;
 if(*(session+0x54)!=0)return;
 auto* gateObj=*reinterpret_cast<std::uint8_t**>(0x011E8804);auto** vt=*reinterpret_cast<void***>(gateObj);using GateFn=int(__thiscall*)(void*);if(reinterpret_cast<GateFn>(vt[0x38/sizeof(void*)])(gateObj)!=0)return;
 auto flags=*reinterpret_cast<std::uint32_t*>(root+0x10438);
 if((flags&0x20000000u)!=0)return;
 ...
 if(DetectControllerJoin_79ADA0(input,&detected)){
   ...
   BeginAddPlayer(self,slot,detected,false);return;
 }
