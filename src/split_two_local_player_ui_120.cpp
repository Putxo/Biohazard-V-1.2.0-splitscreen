...
void UpdateTwoLocalPlayerUi_7DFBA0(void* selfRaw){
 ...
 for(int slot=0;slot<2;++slot){ auto&r=records[slot]; *reinterpret_cast<int*>(gInput_1249C40+0x618)=slot;
   ...
   // Native 0x7DFC31..0x7DFC4D: this flag bypasses both the interactive and
   // timer state machines and goes straight to 0x7DFEB2 (draw if device>=0).
   if((*reinterpret_cast<std::uint32_t*>(gRoot_12340A4+0x1043C)&0x02000000u)!=0){
     if(device>=0)DrawLocalPlayerUi_7DEA10(self,reinterpret_cast<std::uint8_t*>(&r)-1,slot);
     continue;
   }
   ...
   if(r.type==5){
     ...
     if(next>=r.count&&NavUp(mask)){EmitFrontendEvent_79F790(gFrontend_11B1D0C,0x1D,self); void*a=FindPlayerActorByNo_7E6220(gPlayerManager_11B27DC,slot); *reinterpret_cast<std::uint32_t*>(static_cast<std::uint8_t*>(a)+0x2D84)&=~0x10u; ResetUiRoute_7B43C0(gUiInput_11B20C4,1,0); self[0x2F6E0]&=std::uint8_t(~(1u<<slot)); continue;}
     int updated=cursor;
