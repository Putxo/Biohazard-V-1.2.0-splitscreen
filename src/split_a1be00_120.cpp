...
struct Vec3f120 { float x; float y; float z; };
struct SplitUiOwner120 { std::uint8_t _00[0x1AC]; };
                                   ...
                                   const SplitRenderState120* split)
{
    ...
    const int splitX0 = SplitCoordTransform_76A420(const_cast<SplitRenderState120*>(split), 0);
    ...
    for (int player = 0; player < 2; ++player) {
        ...
        for (const auto& e : kEntries) {
            ...
            pos.y = e.useSplitVerticalY
                ? static_cast<float>(SplitVerticalTransform_76A4A0(const_cast<SplitRenderState120*>(split), 0))
                : e.yConstant;
            pos.z = 0.0f;
